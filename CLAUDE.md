...
just register-client       # register PSA-Mokosh-Clients (public OIDC, after `just dev` up)
...
`just dev` requires the shared external `network-traefik-public` to already exist and an Ed25519 keypair at `secrets/dev-key.pem` + `secrets/dev-key.pub.pem`; the keypair is auto-generated by the `ensure-oidc-keys` prerequisite via `scripts/gen-oidc-key.sh dev-key` (each dev machine generates its own; not committed).
