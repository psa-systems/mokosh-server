...
impl ContractsService {
    ...
    pub async fn list_contracts(
        &self, tenant_id: Uuid, filter: &ContractFilter,
    ) -> AppResult<Vec<ContractResponse>> {
        let mut conditions = vec!["tenant_id = $1".to_string()];
        let mut idx = 2;
        if filter.company_id.is_some() { conditions.push(format!("company_id = ${idx}")); idx += 1; }
        if filter.contract_type.is_some() { conditions.push(format!("contract_type = ${idx}")); idx += 1; }
        ...
        if let Some(v) = &filter.contract_type { q = q.bind(v); }
    ...
    pub async fn create_contract(
        &self, tenant_id: Uuid, request: &CreateContractRequest,
        ...
        .bind(&request.contract_number).bind(&request.name).bind(request.company_id)
        .bind(&request.contract_type).bind(&request.status)
        .bind(request.start_date).bind(request.end_date)
        .bind(request.auto_renew).bind(&request.billing_cycle).bind(request.billing_amount)
        .bind(request.sla_id).bind(request.signed_date).bind(request.signed_by_contact_id)
        .bind(&request.notes)
    ...
    pub async fn update_contract(
        &self, tenant_id: Uuid, id: Uuid, request: &UpdateContractRequest,
    ) -> AppResult<ContractResponse> {
        let n = sqlx::query(
        ...
        .bind(&request.contract_number).bind(&request.name).bind(&request.status)
        .bind(request.end_date).bind(request.auto_renew)
        .bind(&request.billing_cycle).bind(request.billing_amount)
        .bind(request.sla_id).bind(request.signed_date).bind(request.signed_by_contact_id)
        .bind(&request.notes)
        .execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("Contract".to_string())); }
        self.get_contract(tenant_id, id).await
    }
    ...
    pub async fn list_contract_items(&self, tenant_id: Uuid, contract_id: Uuid) -> AppResult<Vec<ContractItemResponse>> {
        let rows = sqlx::query_as::<_, ContractItemRow>(
            r#"SELECT id, contract_id, name, description, item_type, quantity, unit_price,
               ...
               ORDER BY sort_order"#,
        ).bind(tenant_id).bind(contract_id).fetch_all(self.db.pool()).await?;
        Ok(rows.into_iter().map(Into::into).collect())
    }

    pub async fn create_contract_item(
        &self, tenant_id: Uuid, contract_id: Uuid, request: &UpsertContractItemRequest,
        ...
        Ok(ContractItemResponse {
            ...
            quantity: request.quantity, unit_price: request.unit_price, total_price: total,
            billing_frequency: request.billing_frequency.clone(),
            work_type_id: request.work_type_id,
            included_hours: request.included_hours, overage_rate: request.overage_rate,
            rollover_enabled: request.rollover_enabled, max_rollover_hours: request.max_rollover_hours,
            sort_order: request.sort_order,
        })
    }

    pub async fn update_contract_item(
        &self, tenant_id: Uuid, id: Uuid, request: &UpsertContractItemRequest,
    ) -> AppResult<ContractItemResponse> {
        let total = request.quantity * request.unit_price;
        ...
        let Some(cid) = contract_id else { return Err(AppError::NotFound("ContractItem".to_string())); };
        Ok(ContractItemResponse {
            id, contract_id: cid,
            name: request.name.clone(), description: request.description.clone(),
            item_type: request.item_type.clone(),
    ...
    pub async fn delete_contract_item(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        let n = sqlx::query("DELETE FROM contract_items WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id).bind(id).execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("ContractItem".to_string())); }
        Ok(())
    }
    ...
    pub async fn create_rate_card(
        &self, tenant_id: Uuid, request: &UpsertRateCardRequest,
        ...
        .bind(id).bind(tenant_id).bind(&request.name).bind(&request.description).bind(request.is_default)
    ...
    pub async fn update_rate_card(
        &self, tenant_id: Uuid, id: Uuid, request: &UpsertRateCardRequest,
    ) -> AppResult<RateCardResponse> {
        let mut tx = self.db.pool().begin().await?;
        if request.is_default {
            sqlx::query("UPDATE rate_cards SET is_default = FALSE WHERE tenant_id = $1 AND id <> $2")
        ...
        if n == 0 { return Err(AppError::NotFound("RateCard".to_string())); }
        tx.commit().await?;
        Ok(RateCardResponse {
            id, name: request.name.clone(), description: request.description.clone(),
            is_default: request.is_default,
    ...
    pub async fn list_rate_card_items(&self, tenant_id: Uuid, rate_card_id: Uuid) -> AppResult<Vec<RateCardItemResponse>> {
        ...
        ).bind(tenant_id).bind(rate_card_id).fetch_all(self.db.pool()).await?;
        Ok(rows.into_iter().map(Into::into).collect())
    }

    pub async fn upsert_rate_card_item(
        &self, tenant_id: Uuid, rate_card_id: Uuid, request: &UpsertRateCardItemRequest,
        ...
        ).bind(rate_card_id).bind(tenant_id).fetch_one(self.db.pool()).await?;
        if !exists { return Err(AppError::NotFound("RateCard".to_string())); }
        ...
        .bind(rate_card_id).bind(request.work_type_id)
        .bind(request.hourly_rate).bind(request.after_hours_rate).bind(request.emergency_rate)
        .fetch_one(self.db.pool()).await?;
        Ok(RateCardItemResponse {
            id, rate_card_id, work_type_id: request.work_type_id,
    ...
    pub async fn delete_rate_card_item(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        ...
        if n == 0 { return Err(AppError::NotFound("RateCardItem".to_string())); }
        Ok(())
    }
}
...
#[derive(sqlx::FromRow)]
struct ContractRow {
    id: Uuid, contract_number: Option<String>, name: String, company_id: Uuid,
    contract_type: String, status: Option<String>,
    start_date: chrono::NaiveDate, end_date: Option<chrono::NaiveDate>,
    auto_renew: Option<bool>, billing_cycle: Option<String>,
    billing_amount: Option<Decimal>, sla_id: Option<Uuid>,
    signed_date: Option<chrono::NaiveDate>, signed_by_contact_id: Option<Uuid>,
    notes: Option<String>,
...
impl From<ContractRow> for ContractResponse {
    fn from(r: ContractRow) -> Self {
        Self {
            id: r.id, contract_number: r.contract_number, name: r.name, company_id: r.company_id,
            contract_type: r.contract_type, status: r.status.unwrap_or_else(|| "draft".into()),
            start_date: r.start_date, end_date: r.end_date,
            auto_renew: r.auto_renew.unwrap_or(false),
            billing_cycle: r.billing_cycle.unwrap_or_else(|| "monthly".into()),
            billing_amount: r.billing_amount, sla_id: r.sla_id,
            signed_date: r.signed_date, signed_by_contact_id: r.signed_by_contact_id,
...
struct ContractItemRow {
    id: Uuid, contract_id: Uuid, name: String, description: Option<String>,
    item_type: String,
    quantity: Decimal, unit_price: Decimal, total_price: Decimal,
    billing_frequency: Option<String>,
    work_type_id: Option<Uuid>,
    included_hours: Option<Decimal>, overage_rate: Option<Decimal>,
    rollover_enabled: Option<bool>, max_rollover_hours: Option<Decimal>,
...
impl From<ContractItemRow> for ContractItemResponse {
    fn from(r: ContractItemRow) -> Self {
        Self {
            id: r.id, contract_id: r.contract_id, name: r.name, description: r.description,
            ...
            included_hours: r.included_hours, overage_rate: r.overage_rate,
...
#[derive(sqlx::FromRow)]
struct BalanceRow {
    id: Uuid, contract_id: Uuid, contract_item_id: Uuid,
    period_start: chrono::NaiveDate, period_end: chrono::NaiveDate,
    hours_included: Decimal, hours_used: Option<Decimal>, hours_remaining: Decimal,
...
impl From<BalanceRow> for ContractHourBalanceResponse {
    fn from(r: BalanceRow) -> Self {
        Self {
            id: r.id, contract_id: r.contract_id, contract_item_id: r.contract_item_id,
...
#[derive(sqlx::FromRow)]
struct RateCardRow {
    id: Uuid, name: String, description: Option<String>, is_default: Option<bool>,
}
...
struct RateCardItemRow {
    id: Uuid, rate_card_id: Uuid, work_type_id: Uuid,
    hourly_rate: Decimal,
    after_hours_rate: Option<Decimal>, emergency_rate: Option<Decimal>,
}

impl From<RateCardItemRow> for RateCardItemResponse {
    fn from(r: RateCardItemRow) -> Self {
        Self {
            id: r.id, rate_card_id: r.rate_card_id, work_type_id: r.work_type_id,
            hourly_rate: r.hourly_rate,
            after_hours_rate: r.after_hours_rate, emergency_rate: r.emergency_rate,
        }
    }
}
