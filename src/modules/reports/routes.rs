...
pub fn reports_routes(service: ReportsService) -> Router {
    let state = ReportsRouterState { service: Arc::new(service) };
    Router::new()
...
async fn dashboard(
    State(s): State<ReportsRouterState>, RequireAuth(u): RequireAuth,
...
#[derive(Deserialize)]
struct BillingQ { company_id: Option<Uuid> }

async fn billing_report(
    State(s): State<ReportsRouterState>, RequireAuth(u): RequireAuth, _m: RequireManager,
...
fn default_csv() -> String { "csv".into() }
...
async fn export_report(
    State(s): State<ReportsRouterState>, RequireAuth(u): RequireAuth,
    Path(report): Path<String>, Query(q): Query<ExportQ>,
) -> AppResult<Response> {
...
fn csv_for_dashboard(r: &DashboardResponse) -> String {
    ...
    for b in &r.open_by_priority { s.push_str(&format!("{},{}\n", b.label, b.count)); }
    s.push_str(&format!("\nsla_warnings,{}\n", r.sla_warnings));
    s.push_str(&format!("sla_breached,{}\n", r.sla_breached));
    s.push_str("\ndate,opened\n");
    for d in &r.ticket_trend_30d { s.push_str(&format!("{},{}\n", d.date, d.count)); }
    s
}

fn csv_for_tickets(r: &TicketsReportResponse) -> String {
    let mut s = format!("from,to\n{},{}\n\nstatus,opened\n", r.from, r.to);
    for b in &r.opened_by_status { s.push_str(&format!("{},{}\n", b.label, b.count)); }
    s.push_str(&format!("\nclosed_total,{}\n\nassignee_id,opened\n", r.closed_total));
    for a in &r.opened_by_assignee {
        s.push_str(&format!(
            "{},{}\n",
            a.assignee_id.map(|u| u.to_string()).unwrap_or_else(|| "unassigned".into()),
            a.count,
        ));
    }
    s
...
fn csv_for_time(r: &TimeReportResponse) -> String {
    let mut s = format!("from,to\n{},{}\n\nuser_id,minutes\n", r.from, r.to);
    for i in &r.minutes_by_user { s.push_str(&format!("{},{}\n", i.id, i.count)); }
    s.push_str("\nwork_type_id,minutes\n");
    for i in &r.minutes_by_work_type { s.push_str(&format!("{},{}\n", i.id, i.count)); }
    s
}
...
fn csv_for_billing(r: &BillingReportResponse) -> String {
    ...
    s.push_str("\nbucket,total\n");
    for b in &r.aging { s.push_str(&format!("{},{}\n", b.bucket, b.total)); }
    s
}
