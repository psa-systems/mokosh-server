...
impl ReportsService {
    ...
    pub async fn dashboard(&self, tenant_id: Uuid) -> AppResult<DashboardResponse> {
        ...
        Ok(DashboardResponse {
            open_by_priority: open_by_priority.into_iter()
                .map(|(name, count)| Bucket { label: name, count }).collect(),
            sla_warnings, sla_breached,
            ticket_trend_30d: trend.into_iter()
                .map(|(d, count)| DatedCount { date: d, count }).collect(),
        })
    }

    // PMS-96 tickets ----------------------------------------------------------
    pub async fn tickets(
        &self, tenant_id: Uuid, from: Option<NaiveDate>, to: Option<NaiveDate>,
        ...
        ).bind(tenant_id).bind(from).bind(to).fetch_all(self.db.pool()).await?;
        let closed: i64 = sqlx::query_scalar(
            r#"SELECT COUNT(*)::bigint FROM tickets
               WHERE tenant_id = $1 AND closed_at::date BETWEEN $2 AND $3"#,
        ).bind(tenant_id).bind(from).bind(to).fetch_one(self.db.pool()).await?;
        ...
        Ok(TicketsReportResponse {
            from, to,
            opened_by_status: opened.into_iter().map(|(name, c)| Bucket { label: name, count: c }).collect(),
            closed_total: closed,
            opened_by_assignee: by_assignee.into_iter().map(|(uid, c)| AssigneeCount {
                assignee_id: uid, count: c,
            }).collect(),
        })
    }
    ...
    pub async fn time(
        ...
        Ok(TimeReportResponse {
            from, to,
            minutes_by_user: by_user.into_iter().map(|(id, m)| IdCount { id, count: m }).collect(),
            minutes_by_work_type: by_work_type.into_iter().map(|(id, m)| IdCount { id, count: m }).collect(),
        })
    }

    // PMS-98 billing ----------------------------------------------------------
    pub async fn billing(
        &self, tenant_id: Uuid, company_id: Option<Uuid>,
        ...
        let totals: Option<(Decimal, Decimal, Decimal)> = if let Some(c) = company_id {
            ...
            ).bind(tenant_id).bind(c).fetch_optional(self.db.pool()).await?
        } else {
            sqlx::query_as(
                r#"SELECT COALESCE(SUM(total), 0), COALESCE(SUM(amount_paid), 0),
                          COALESCE(SUM(balance_due), 0)
        ...
        let (invoiced, paid, outstanding) = totals.unwrap_or((Decimal::ZERO, Decimal::ZERO, Decimal::ZERO));
        ...
        let aging: Vec<(String, Decimal)> = sqlx::query_as(
            r#"SELECT
               ...
               GROUP BY bucket"#,
        ).bind(tenant_id).bind(company_id).fetch_all(self.db.pool()).await?;

        Ok(BillingReportResponse {
            invoiced, paid, outstanding,
            aging: aging.into_iter().map(|(bucket, total)| AgingBucket {
                bucket, total,
            }).collect(),
        })
    }
...
#[derive(Debug, Clone, Serialize)]
pub struct DatedCount { pub date: NaiveDate, pub count: i64 }

#[derive(Debug, Clone, Serialize)]
pub struct AssigneeCount { pub assignee_id: Option<Uuid>, pub count: i64 }

#[derive(Debug, Clone, Serialize)]
pub struct IdCount { pub id: Uuid, pub count: i64 }

#[derive(Debug, Clone, Serialize)]
pub struct AgingBucket { pub bucket: String, pub total: Decimal }
