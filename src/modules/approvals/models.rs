...
pub struct ApprovalResponse {
    ...
    /// third approver kind beside a user and a role (migration 143).
    /// The contact plane lists and decides exactly these rows.
    pub approver_contact_id: Option<Uuid>,
    pub approver_contact_name: Option<String>,
    pub status: String,
    pub notes: Option<String>,
    pub decision_notes: Option<String>,
    pub decided_by_id: Option<Uuid>,
    pub decided_by_name: Option<String>,
    /// PMS-1084: the contact who decided on the contact plane. NULL
    /// while pending and on a staff decision.
    pub decided_by_contact_id: Option<Uuid>,
    pub decided_by_contact_name: Option<String>,
...
pub struct CreateApprovalRequest {
    /// Exactly one of `approver_user_id` (assign-by-id),
    /// `approver_role` (assign-by-role) or `approver_contact_id`
    /// (PMS-1084: address it to a customer, who decides it on the
    /// contact plane) must be set. Any other count is a 400 surfaced
    /// from the service before reaching the DB CHECK so the SPA gets
