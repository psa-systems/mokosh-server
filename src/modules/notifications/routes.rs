...
pub fn notifications_routes(service: NotificationsService) -> Router {
    let state = NotificationsRouterState { service: Arc::new(service) };
    Router::new()
        // PMS-87 channels
        .route("/notification-channels", get(list_channels).post(create_channel))
        .route("/notification-channels/{id}", axum::routing::delete(delete_channel))
        // PMS-88 templates
        .route("/notification-templates", get(list_templates).post(create_template))
        .route("/notification-templates/{id}", axum::routing::delete(delete_template))
        // PMS-89 user prefs
        .route("/me/notification-preferences", get(list_user_prefs).put(upsert_user_pref))
        // PMS-90 inbox
        .route("/notifications", get(list_inbox))
        .route("/notifications/{id}/read", post(mark_read))
        // PMS-91 rules
        .route("/notification-rules", get(list_rules).post(create_rule))
        .route("/notification-rules/{id}", axum::routing::delete(delete_rule))
...
async fn list_channels(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
...
async fn delete_channel(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_channel(u.tenant_id, id).await }

async fn list_templates(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth,
...
async fn delete_template(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_template(u.tenant_id, id).await }

async fn list_user_prefs(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth,
) -> AppResult<Json<Vec<UserNotificationPreferenceResponse>>> {
    Ok(Json(s.service.list_user_preferences(u.tenant_id, u.id).await?))
}
...
async fn upsert_user_pref(
    ...
    req.validate()?;
    Ok(Json(s.service.upsert_user_preference(u.tenant_id, u.id, &req).await?))
}
...
async fn mark_read(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.mark_read(u.tenant_id, u.id, id).await }
...
async fn delete_rule(
    State(s): State<NotificationsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_rule(u.tenant_id, id).await }
...
async fn dispatch_event(
    ...
    req.validate()?;
    let count = s.service.dispatch(u.tenant_id, &req.event_type, &req.context).await?;
