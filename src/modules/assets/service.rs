...
impl AssetsService {
    ...
    pub async fn create_asset_type(
        &self, tenant_id: Uuid, request: &UpsertAssetTypeRequest,
        ...
        .bind(id).bind(tenant_id).bind(&request.name).bind(&request.icon)
        .bind(request.parent_type_id).bind(request.is_active)
        .execute(self.db.pool()).await?;
        Ok(AssetTypeResponse {
            id, name: request.name.clone(), icon: request.icon.clone(),
            parent_type_id: request.parent_type_id, is_active: request.is_active,
        })
    }

    pub async fn update_asset_type(
        &self, tenant_id: Uuid, id: Uuid, request: &UpsertAssetTypeRequest,
    ) -> AppResult<AssetTypeResponse> {
        let n = sqlx::query(
        ...
        .bind(tenant_id).bind(id).bind(&request.name).bind(&request.icon)
        .bind(request.parent_type_id).bind(request.is_active)
        .execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("AssetType".to_string())); }
        Ok(AssetTypeResponse {
            id, name: request.name.clone(), icon: request.icon.clone(),
            parent_type_id: request.parent_type_id, is_active: request.is_active,
        })
    ...
    pub async fn list_assets(
        &self, tenant_id: Uuid, filter: &AssetFilter,
    ) -> AppResult<Vec<AssetResponse>> {
        let mut conditions = vec!["tenant_id = $1".to_string()];
        let mut idx = 2;
        if filter.company_id.is_some() { conditions.push(format!("company_id = ${idx}")); idx += 1; }
        if filter.asset_type_id.is_some() { conditions.push(format!("asset_type_id = ${idx}")); idx += 1; }
        if filter.status.is_some() { conditions.push(format!("status = ${idx}")); }
        ...
        if let Some(v) = filter.company_id { q = q.bind(v); }
        if let Some(v) = filter.asset_type_id { q = q.bind(v); }
    ...
    pub async fn create_asset(
        &self, tenant_id: Uuid, performer: Uuid, request: &CreateAssetRequest,
        ...
        .bind(id).bind(tenant_id).bind(&request.asset_tag).bind(&request.name)
        .bind(request.asset_type_id).bind(request.company_id)
        .bind(request.site_id).bind(request.contact_id).bind(&request.status)
        .bind(&request.manufacturer).bind(&request.model).bind(&request.serial_number)
        .bind(request.purchase_date).bind(request.purchase_price)
        .bind(request.warranty_expiry).bind(request.end_of_life)
        .execute(&mut *tx).await?;
        sqlx::query(
            r#"INSERT INTO asset_audit_log (tenant_id, asset_id, action, performed_by_id)
               VALUES ($1, $2, 'created', $3)"#,
        ).bind(tenant_id).bind(id).bind(performer).execute(&mut *tx).await?;
        tx.commit().await?;
    ...
    pub async fn update_asset(
        &self, tenant_id: Uuid, id: Uuid, performer: Uuid, request: &UpdateAssetRequest,
    ) -> AppResult<AssetResponse> {
        ...
        .bind(&request.asset_tag).bind(&request.name).bind(request.asset_type_id)
        .bind(request.site_id).bind(request.contact_id).bind(&request.status)
        .bind(&request.manufacturer).bind(&request.model).bind(&request.serial_number)
        .bind(request.purchase_date).bind(request.purchase_price)
        .bind(request.warranty_expiry).bind(request.end_of_life)
        .execute(&mut *tx).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("Asset".to_string())); }
        ...
        sqlx::query(
            r#"INSERT INTO asset_audit_log (tenant_id, asset_id, action, performed_by_id)
               VALUES ($1, $2, $3, $4)"#,
        ).bind(tenant_id).bind(id).bind(action).bind(performer).execute(&mut *tx).await?;
        tx.commit().await?;
        self.get_asset(tenant_id, id).await
    }
    ...
    pub async fn list_asset_relationships(
        ...
        ).bind(tenant_id).bind(asset_id).fetch_all(self.db.pool()).await?;
        Ok(rows.into_iter().map(Into::into).collect())
    }

    pub async fn create_asset_relationship(
        &self, tenant_id: Uuid, parent_id: Uuid, request: &CreateAssetRelationshipRequest,
        ...
        .bind(id).bind(tenant_id).bind(parent_id).bind(request.child_asset_id)
        .bind(&request.relationship_type)
        .execute(self.db.pool()).await?;
        Ok(AssetRelationshipResponse {
            id, parent_asset_id: parent_id, child_asset_id: request.child_asset_id,
    ...
    pub async fn delete_asset_relationship(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        let n = sqlx::query("DELETE FROM asset_relationships WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id).bind(id).execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("AssetRelationship".to_string())); }
        Ok(())
    }
    ...
    pub async fn list_configuration_items(
        ...
        rows.into_iter().map(|r| {
            ...
            Ok(ConfigurationItemResponse {
                ...
                name: r.name, category: r.category, value, notes: r.notes,
    ...
    pub async fn upsert_configuration_item(
        &self, tenant_id: Uuid, asset_id: Uuid, request: &UpsertConfigurationItemRequest,
        ...
        .bind(id).bind(tenant_id).bind(asset_id)
        .bind(&request.name).bind(&request.category).bind(&encrypted).bind(&request.notes)
        .execute(self.db.pool()).await?;
        Ok(ConfigurationItemResponse {
            id, asset_id,
            name: request.name.clone(), category: request.category.clone(),
            value: request.value.clone(), notes: request.notes.clone(),
    ...
    pub async fn delete_configuration_item(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        let n = sqlx::query("DELETE FROM configuration_items WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id).bind(id).execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("ConfigurationItem".to_string())); }
        Ok(())
    }

    // PMS-77 credential vault ------------------------------------------------
    pub async fn list_credentials(
        &self, tenant_id: Uuid, asset_id: Uuid, performer: Uuid,
        ...
        ).bind(tenant_id).bind(asset_id).bind(performer)
        .execute(self.db.pool()).await?;
        ...
        rows.into_iter().map(|r| {
            ...
            Ok(CredentialResponse {
                id: r.id, name: r.name, company_id: r.company_id, asset_id: r.asset_id,
                credential_type: r.credential_type,
                username, password, url: r.url, notes,
                last_rotated: r.last_rotated, created_at: r.created_at,
            })
        }).collect()
    }

    pub async fn create_credential(
        &self, tenant_id: Uuid, asset_id: Uuid, performer: Uuid, request: &CreateCredentialRequest,
        ...
        let company_id: Option<Uuid> = sqlx::query_scalar(
            "SELECT company_id FROM assets WHERE id = $1 AND tenant_id = $2",
        ).bind(asset_id).bind(tenant_id).fetch_optional(self.db.pool()).await?;
        let id = Uuid::new_v4();
        let mut tx = self.db.pool().begin().await?;
        sqlx::query(
        ...
        .bind(id).bind(tenant_id).bind(&request.name).bind(company_id).bind(asset_id)
        .bind(&request.credential_type).bind(&username_enc).bind(&password_enc)
        .bind(&request.url).bind(&notes_enc)
        .execute(&mut *tx).await?;
        sqlx::query(
            r#"INSERT INTO asset_audit_log (tenant_id, asset_id, action, performed_by_id,
                                            changes)
               VALUES ($1, $2, 'updated', $3, '{"event":"credential_created"}'::jsonb)"#,
        ).bind(tenant_id).bind(asset_id).bind(performer).execute(&mut *tx).await?;
        tx.commit().await?;

        Ok(CredentialResponse {
            id, name: request.name.clone(), company_id, asset_id: Some(asset_id),
            credential_type: request.credential_type.clone(),
            username: request.username.clone(), password: request.password.clone(),
            url: request.url.clone(), notes: request.notes.clone(),
            last_rotated: None, created_at: chrono::Utc::now(),
        })
    }
    ...
    pub async fn delete_credential(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        let n = sqlx::query("DELETE FROM credential_vault WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id).bind(id).execute(self.db.pool()).await?.rows_affected();
        if n == 0 { return Err(AppError::NotFound("Credential".to_string())); }
        Ok(())
    }
...
#[derive(sqlx::FromRow)]
struct AssetTypeRow {
    id: Uuid, name: String, icon: Option<String>,
    parent_type_id: Option<Uuid>, is_active: Option<bool>,
}
...
impl From<AssetTypeRow> for AssetTypeResponse {
    fn from(r: AssetTypeRow) -> Self {
        Self {
            ...
            parent_type_id: r.parent_type_id, is_active: r.is_active.unwrap_or(true),
        }
    }
}
...
#[derive(sqlx::FromRow)]
struct AssetRow {
    id: Uuid, asset_tag: Option<String>, name: String,
    asset_type_id: Uuid, company_id: Uuid,
    site_id: Option<Uuid>, contact_id: Option<Uuid>,
    status: Option<String>,
    manufacturer: Option<String>, model: Option<String>, serial_number: Option<String>,
    purchase_date: Option<chrono::NaiveDate>, purchase_price: Option<Decimal>,
    warranty_expiry: Option<chrono::NaiveDate>, end_of_life: Option<chrono::NaiveDate>,
...
impl From<AssetRow> for AssetResponse {
    fn from(r: AssetRow) -> Self {
        Self {
            id: r.id, asset_tag: r.asset_tag, name: r.name,
            asset_type_id: r.asset_type_id, company_id: r.company_id,
            site_id: r.site_id, contact_id: r.contact_id,
            status: r.status.unwrap_or_else(|| "active".into()),
            manufacturer: r.manufacturer, model: r.model, serial_number: r.serial_number,
            purchase_date: r.purchase_date, purchase_price: r.purchase_price,
            warranty_expiry: r.warranty_expiry, end_of_life: r.end_of_life,
...
#[derive(sqlx::FromRow)]
struct RelRow {
    id: Uuid, parent_asset_id: Uuid, child_asset_id: Uuid, relationship_type: String,
}
...
impl From<RelRow> for AssetRelationshipResponse {
    fn from(r: RelRow) -> Self {
        Self {
            ...
            child_asset_id: r.child_asset_id, relationship_type: r.relationship_type,
        }
    }
}
...
struct ConfigItemRow {
    id: Uuid, asset_id: Uuid, name: String, category: Option<String>,
    value_encrypted: String, notes: Option<String>,
...
struct CredRow {
    id: Uuid, name: String,
    company_id: Option<Uuid>, asset_id: Option<Uuid>,
    credential_type: String,
    username_encrypted: String, password_encrypted: String,
    url: Option<String>, notes_encrypted: Option<String>,
...
#[derive(sqlx::FromRow)]
struct AuditRow {
    id: Uuid, asset_id: Uuid, action: String,
    changes: Option<serde_json::Value>, performed_by_id: Option<Uuid>,
...
impl From<AuditRow> for AssetAuditLogResponse {
    fn from(r: AuditRow) -> Self {
        Self {
            ...
            changes: r.changes, performed_by_id: r.performed_by_id,
