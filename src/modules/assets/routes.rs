...
pub fn assets_routes(service: AssetsService) -> Router {
    let state = AssetsRouterState { service: Arc::new(service) };
    Router::new()
        // PMS-73 asset types
        .route("/asset-types", get(list_asset_types).post(create_asset_type))
        .route("/asset-types/{id}", put(update_asset_type).delete(delete_asset_type))
        // PMS-74 assets
        ...
        .route("/asset-relationships/{id}", axum::routing::delete(delete_asset_relationship))
        // PMS-76 configuration items
        .route(
            "/assets/{id}/configuration-items",
            get(list_configuration_items).post(create_configuration_item),
        )
        .route("/configuration-items/{id}", axum::routing::delete(delete_configuration_item))
...
async fn list_asset_types(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth,
) -> AppResult<Json<Vec<AssetTypeResponse>>> {
    Ok(Json(s.service.list_asset_types(u.tenant_id).await?))
}

async fn create_asset_type(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
...
async fn update_asset_type(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>, Json(req): Json<UpsertAssetTypeRequest>,
) -> AppResult<Json<AssetTypeResponse>> {
    req.validate()?;
    Ok(Json(s.service.update_asset_type(u.tenant_id, id, &req).await?))
}

async fn delete_asset_type(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_asset_type(u.tenant_id, id).await }
...
async fn get_asset(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<Json<AssetResponse>> {
...
async fn update_asset(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth,
    Path(id): Path<Uuid>, Json(req): Json<UpdateAssetRequest>,
) -> AppResult<Json<AssetResponse>> {
    req.validate()?;
    Ok(Json(s.service.update_asset(u.tenant_id, id, u.id, &req).await?))
}

async fn delete_asset(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_asset(u.tenant_id, id).await }

async fn list_asset_relationships(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<Json<Vec<AssetRelationshipResponse>>> {
    Ok(Json(s.service.list_asset_relationships(u.tenant_id, id).await?))
}

async fn create_asset_relationship(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth,
    Path(id): Path<Uuid>, Json(req): Json<CreateAssetRelationshipRequest>,
) -> AppResult<Json<AssetRelationshipResponse>> {
    req.validate()?;
    Ok(Json(s.service.create_asset_relationship(u.tenant_id, id, &req).await?))
}

async fn delete_asset_relationship(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_asset_relationship(u.tenant_id, id).await }

async fn list_configuration_items(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<Json<Vec<ConfigurationItemResponse>>> {
    Ok(Json(s.service.list_configuration_items(u.tenant_id, id).await?))
}

async fn create_configuration_item(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth,
    Path(id): Path<Uuid>, Json(req): Json<UpsertConfigurationItemRequest>,
) -> AppResult<Json<ConfigurationItemResponse>> {
    req.validate()?;
    Ok(Json(s.service.upsert_configuration_item(u.tenant_id, id, &req).await?))
}

async fn delete_configuration_item(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_configuration_item(u.tenant_id, id).await }

async fn list_credentials(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<Json<Vec<CredentialResponse>>> {
    Ok(Json(s.service.list_credentials(u.tenant_id, id, u.id).await?))
}

async fn create_credential(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth,
    Path(id): Path<Uuid>, Json(req): Json<CreateCredentialRequest>,
) -> AppResult<Json<CredentialResponse>> {
    req.validate()?;
    Ok(Json(s.service.create_credential(u.tenant_id, id, u.id, &req).await?))
}

async fn delete_credential(
    State(s): State<AssetsRouterState>, RequireAuth(u): RequireAuth, Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_credential(u.tenant_id, id).await }
