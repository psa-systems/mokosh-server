...
pub fn settings_routes(service: SettingsService) -> Router {
    let state = SettingsRouterState { service: Arc::new(service) };
    Router::new()
        ...
        .route("/settings/modules/{module}", get(get_module_config).put(upsert_module_config))
        .with_state(state)
}

async fn list_settings(
    State(s): State<SettingsRouterState>, RequireAuth(u): RequireAuth,
) -> AppResult<Json<Vec<TenantSettingResponse>>> {
    Ok(Json(s.service.list_tenant_settings(u.tenant_id).await?))
}

async fn upsert_setting(
    State(s): State<SettingsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Json(req): Json<UpsertTenantSettingRequest>,
) -> AppResult<Json<TenantSettingResponse>> {
    req.validate()?;
    Ok(Json(s.service.upsert_tenant_setting(u.tenant_id, &req).await?))
}

async fn delete_setting(
    State(s): State<SettingsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(id): Path<Uuid>,
) -> AppResult<()> { s.service.delete_tenant_setting(u.tenant_id, id).await }
...
async fn get_module_config(
    State(s): State<SettingsRouterState>, RequireAuth(u): RequireAuth, Path(module): Path<String>,
) -> AppResult<Json<ModuleConfigResponse>> {
    Ok(Json(s.service.get_module_config(u.tenant_id, &module).await?))
}

async fn upsert_module_config(
    State(s): State<SettingsRouterState>, RequireAuth(u): RequireAuth, _a: RequireAdmin,
    Path(module): Path<String>, Json(req): Json<UpsertModuleConfigRequest>,
) -> AppResult<Json<ModuleConfigResponse>> {
    req.validate()?;
    Ok(Json(s.service.upsert_module_config(u.tenant_id, &module, &req).await?))
}
