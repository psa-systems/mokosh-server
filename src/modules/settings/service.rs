...
impl SettingsService {
    ...
    pub async fn list_tenant_settings(&self, tenant_id: Uuid) -> AppResult<Vec<TenantSettingResponse>> {
    ...
    pub async fn upsert_tenant_setting(
        &self, tenant_id: Uuid, request: &UpsertTenantSettingRequest,
        ...
        .bind(tenant_id).bind(&request.category).bind(&request.key).bind(&request.value)
        .fetch_one(self.db.pool()).await?;
        Ok(TenantSettingResponse {
            id, category: request.category.clone(), key: request.key.clone(),
    ...
    pub async fn delete_tenant_setting(&self, tenant_id: Uuid, id: Uuid) -> AppResult<()> {
        ...
        if n == 0 { return Err(AppError::NotFound("TenantSetting".to_string())); }
        Ok(())
    }
    ...
    pub async fn list_module_configs(&self, tenant_id: Uuid) -> AppResult<Vec<ModuleConfigResponse>> {
    ...
    pub async fn get_module_config(
        ...
        ).bind(tenant_id).bind(module).fetch_optional(self.db.pool()).await?
        .ok_or_else(|| AppError::NotFound("ModuleConfig".to_string()))?;
        Ok(row.into())
    }

    pub async fn upsert_module_config(
        &self, tenant_id: Uuid, module: &str, request: &UpsertModuleConfigRequest,
    ) -> AppResult<ModuleConfigResponse> {
        let id: Uuid = sqlx::query_scalar(
        ...
        .bind(tenant_id).bind(module).bind(request.is_enabled).bind(&request.config)
        .fetch_one(self.db.pool()).await?;
        Ok(ModuleConfigResponse {
            id, module_name: module.to_string(),
            is_enabled: request.is_enabled, config: request.config.clone(),
        })
    }
}
...
#[derive(sqlx::FromRow)]
struct SettingRow {
    id: Uuid, category: String, key: String, value: serde_json::Value,
}

impl From<SettingRow> for TenantSettingResponse {
    fn from(r: SettingRow) -> Self {
        Self { id: r.id, category: r.category, key: r.key, value: r.value }
    }
}

#[derive(sqlx::FromRow)]
struct ModCfgRow {
    id: Uuid, module_name: String, is_enabled: Option<bool>, config: serde_json::Value,
}

impl From<ModCfgRow> for ModuleConfigResponse {
    fn from(r: ModCfgRow) -> Self {
        Self {
            id: r.id, module_name: r.module_name,
