...
mod tests {
    ...
    /// AC4: business, portal and diagnostic mail is not identity mail, and
    /// must be untouched by the mode in both directions.
    ///
    /// The regression worth guarding against is a gate placed one level too
    /// high. Put the check in `NotificationsService::dispatch`, in the
    /// dispatcher worker, or behind the `Mailer` trait, and a SaaS tenant
    /// silently stops sending its customers quotes and invoices - a far larger
    /// failure than the one PMS-904 set out to fix, and one nothing would
    /// report.
    ///
    /// A source scan rather than fifteen behavioural tests, because what is
    /// being pinned is exactly a negative about WHERE the knowledge lives:
    /// that no module other than the auth service and the startup wiring can
    /// even ask which mode this is. Each of those fifteen dispatch sites
    /// already has its own suite; a second copy here would pin their
    /// behaviour, not this boundary.
    ///
    /// Kept as a lib test so `cargo test --workspace --lib` runs it: this is
    /// a static source scan with no database, so `check.yml` reports an
    /// offender in seconds rather than waiting on `integration.yml`'s
    ...
    fn only_the_auth_service_and_the_startup_wiring_know_the_deployment_mode() {
        ...
        let permitted = [
            ...
            // Re-export only.
            "src/utils/mod.rs",
            // PMS-982: names the variable in the stated reason for its entry
            // in `config::guard::ENTRY_POINTS` (it locates the providers, so
            // it cannot be served by one). Prose, not a consultation: nothing
            // in `src/config/` asks which mode this is, which is why the
            // module takes a provider NAME rather than a `DeploymentMode`.
            "src/config/guard.rs",
            // PMS-983: `FlagDefault::PerProfile` needs the hosting profile to
            // resolve `ORGANIZATIONS_ENABLED` at read time. Not a mail gate,
            // not a duplicated auth decision: the profile IS one input to the
            // flag's default.
            "src/config/flags.rs",
            // PMS-981: the authentication provider selection reads the
            // profile's default set. Adjacent to `AuthService` but distinct:
            // this module OWNS the enable/available shape the profile drives.
            "src/modules/auth/providers/mod.rs",
            // PMS-989: the status collector reports the hosting profile in
            // the presence matrix, and names every per-kind deviation from
            // it. That reporting is what makes the profile visible to an
            // operator.
            "src/providers/status/mod.rs",
            // PMS-1012: the operator CLI resolves the profile's default
            // provider per kind when the caller does not name one, so
            // `provider-status` and its siblings behave the same as the boot
            // record.
            ...
            // resolves it exactly as the startup wiring does. Not a
            // duplicated auth decision, not a mail gate: it is the startup
            // wiring reused in a one-shot the operator runs.
            "src/cli/verify.rs",
        ];
        ...
        while let Some(dir) = stack.pop() {
            for entry in std::fs::read_dir(&dir).expect("read src") {
                ...
                let rel = path
                    ...
                    .expect("source path is under the manifest dir")
                    .to_string_lossy()
                    .replace('\\', "/");
