...
//!
//! The portal-feed visibility logic (public vs client_specific scoped by
//! the contact's company) is asserted at the service level in
//! `portal_feed_company_scoping` via a direct `KbService` call, because
//! driving the portal JWT auth in-test requires seeding a contact with a
//! portal password hash and is exercised by the portal's own suite; the
//! service call pins the SQL visibility/scoping that the portal route
//! delegates to.
