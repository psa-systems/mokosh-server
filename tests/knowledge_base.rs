...
//!
//! The portal-visible feed has no HTTP route since PMS-1025 retired the
//! portal tree (the agent tree never carried a `/kb/articles/portal`
//! route either, and the contact plane has no KB route yet, though
//! `kb:read` is still in the capability catalog). The end-to-end case
//! that drove the retired route (401 without a bearer; only the contact's
//! tenant + company published portal-visible articles with one) is
//! recorded on PMS-1064 as coverage the cut removed.
//! `portal_feed_company_scoping` pins the SQL visibility/scoping the
//! future route delegates to, at the service level via a direct
//! `KbService` call.
...
/// Service-level pin for the portal feed visibility rules (PMS-84). The
/// HTTP half went with the PMS-1025 cut; see the module comment.
///
