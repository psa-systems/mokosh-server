...
async fn create_team_persists_row(pool: PgPool) {
    ...
    let team = s.create_team(tenant(), &create_req("Alpha"), &ctx())
        .await
        .expect("create");
...
async fn create_team_returns_full_team_including_timestamps(pool: PgPool) {
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Bravo"), &ctx())
        .await
        .expect("create");
...
async fn list_teams_returns_only_tenant_teams(pool: PgPool) {
    ...
    s.create_team(tenant(), &create_req("From-A"), &ctx()).await.unwrap();
    s.create_team(TenantId::from_trusted(tenant_b), &create_req("From-B"), &ctx())
        .await
        .unwrap();

    let a_teams = s.list_teams(tenant(), TeamListFilters::default()).await.unwrap();
...
async fn list_teams_filters_by_active(pool: PgPool) {
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Charlie"), &ctx()).await.unwrap();
    s.soft_delete_team(tenant(), team.id, &ctx()).await.unwrap();

    let active = s.list_teams(tenant(), TeamListFilters::default()).await.unwrap();
    assert!(!active.iter().any(|t| t.id == team.id));

    let all = s.list_teams(tenant(),
        TeamListFilters { include_inactive: true, ..Default::default() }
    ).await.unwrap();
...
async fn get_team_with_members_includes_join(pool: PgPool) {
    ...
    let s = svc(pool);
    let bundle = s.get_team_with_members(tenant(), team_id).await.expect("bundle");
...
async fn update_team_partial_updates_only_supplied_fields(pool: PgPool) {
    ...
    let updated = s.update_team(tenant(), team.id, &upd, &ctx()).await.unwrap();
...
async fn update_team_rename_to_same_name_is_idempotent(pool: PgPool) {
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Foxtrot"), &ctx()).await.unwrap();
    let upd = UpdateTeamRequest {
    ...
    let updated = s.update_team(tenant(), team.id, &upd, &ctx()).await.expect("self rename");
...
async fn soft_delete_team_flips_is_active_and_preserves_ticket_fk(pool: PgPool) {
    let s = svc(pool.clone());
    let team = s.create_team(tenant(), &create_req("Golf"), &ctx()).await.unwrap();
    s.soft_delete_team(tenant(), team.id, &ctx()).await.expect("soft delete");
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM teams WHERE id = $1")
            .bind(team.id)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn add_member_default_role_is_member(pool: PgPool) {
    ...
    let team = s.create_team(tenant(), &create_req("Hotel"), &ctx()).await.unwrap();
    let m = s.add_member(tenant(), team.id, &AddTeamMemberRequest {
        user_id: admin_id, role: None,
    }, &ctx()).await.expect("add");
...
async fn add_member_explicit_leader_role(pool: PgPool) {
    let s = svc(pool.clone());
    let (admin_id, _e, _p) = common::seed_admin(&pool).await;
    let team = s.create_team(tenant(), &create_req("India"), &ctx()).await.unwrap();
    let m = s.add_member(tenant(), team.id, &AddTeamMemberRequest {
        user_id: admin_id, role: Some("leader".to_string()),
    }, &ctx()).await.expect("add");
...
async fn list_members_returns_joined_user_fields(pool: PgPool) {
    ...
    let m = members.iter().find(|m| m.user_id == admin_id).expect("admin in list");
...
async fn update_member_role_flips_leader_to_member(pool: PgPool) {
    ...
    let s = svc(pool);
    let m = s.update_member_role(tenant(), team_id, admin_id, &UpdateTeamMemberRoleRequest {
        role: "member".to_string(),
    }, &ctx()).await.expect("role update");
    assert_eq!(m.role, "member");
}
...
async fn remove_member_is_idempotent(pool: PgPool) {
    ...
    let s = svc(pool);
    s.remove_member(tenant(), team_id, admin_id, &ctx()).await.expect("first remove");
    s.remove_member(tenant(), team_id, admin_id, &ctx()).await.expect("second remove idempotent");
}
...
async fn list_teams_for_user_returns_only_their_teams(pool: PgPool) {
    ...
    let s = svc(pool.clone());
    let team_b = s.create_team(tenant(), &create_req("Mike-B"), &ctx()).await.unwrap();
...
async fn create_team_duplicate_name_returns_409(pool: PgPool) {
    let s = svc(pool);
    s.create_team(tenant(), &create_req("Nomad"), &ctx()).await.unwrap();
    let err = s.create_team(tenant(), &create_req("NOMAD"), &ctx()).await.unwrap_err();
    assert!(format!("{err:?}").contains("name already exists"), "{err:?}");
}
...
async fn create_team_can_reuse_archived_name(pool: PgPool) {
    let s = svc(pool);
    let first = s.create_team(tenant(), &create_req("Oscar"), &ctx()).await.unwrap();
    s.soft_delete_team(tenant(), first.id, &ctx()).await.unwrap();
...
async fn create_team_missing_name_returns_400(pool: PgPool) {
    ...
    let req = CreateTeamRequest {
        name: String::new(),
        description: None, manager_id: None, color: None,
    };
...
async fn add_member_from_wrong_tenant_returns_400(pool: PgPool) {
    let (tenant_b, user_b, _e, _p) = common::seed_tenant_with_admin(&pool, "wrong-tenant-add").await;
    let s = svc(pool);
    let team_a = s.create_team(tenant(), &create_req("Quebec"), &ctx()).await.unwrap();
    let err = s.add_member(tenant(), team_a.id, &AddTeamMemberRequest {
        user_id: user_b, role: None,
...
async fn update_team_with_wrong_tenant_manager_returns_400(pool: PgPool) {
    ...
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Sierra"), &ctx()).await.unwrap();
    let upd = UpdateTeamRequest {
        name: None, description: None,
        manager_id: Some(user_b),
        color: None, is_active: None,
    };
    let err = s.update_team(tenant(), team.id, &upd, &ctx()).await.unwrap_err();
    assert!(format!("{err:?}").contains("manager_id"), "{err:?}");
}
...
async fn get_team_from_other_tenant_returns_404_not_403(pool: PgPool) {
    ...
    let s = svc(pool);
    let team_b = s.create_team(TenantId::from_trusted(tenant_b), &create_req("Tango"), &ctx())
        .await.unwrap();
...
async fn team_leader_cannot_edit_team_details(pool: PgPool) {
    ...
    let (tech_id, _e, _p) = common::seed_user(
        &pool, common::DEFAULT_TENANT_ID, "tech@example.com", "technician").await;
    let team = s.create_team(tenant(), &create_req("Uniform"), &ctx()).await.unwrap();
    ...
    s.update_team(tenant(), team.id, &UpdateTeamRequest {
        name: Some("Uniform Edited".to_string()),
        description: None, manager_id: None, color: None, is_active: None,
    }, &ctx()).await.expect("service is role-agnostic; route layer gates");
}
...
async fn concurrent_add_same_member_is_safe(pool: PgPool) {
    let (admin_id, _e, _p) = common::seed_admin(&pool).await;
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Yankee"), &ctx()).await.unwrap();
    let req1 = AddTeamMemberRequest { user_id: admin_id, role: None };
    let req2 = AddTeamMemberRequest { user_id: admin_id, role: None };
    let c1 = ctx();
    let c2 = ctx();
    let (a, b) = tokio::join!(
...
async fn every_team_write_appends_audit_log(pool: PgPool) {
    ...
    let team = s.create_team(tenant(), &create_req("Zulu"), &ctx()).await.unwrap();
    s.update_team(tenant(), team.id, &UpdateTeamRequest {
        name: Some("Zulu2".to_string()),
        description: None, manager_id: None, color: None, is_active: None,
    }, &ctx()).await.unwrap();
    s.add_member(tenant(), team.id, &AddTeamMemberRequest {
        user_id: admin_id, role: None,
    }, &ctx()).await.unwrap();
    s.update_member_role(tenant(), team.id, admin_id, &UpdateTeamMemberRoleRequest {
        role: "leader".to_string(),
    }, &ctx()).await.unwrap();
    s.remove_member(tenant(), team.id, admin_id, &ctx()).await.unwrap();
    s.soft_delete_team(tenant(), team.id, &ctx()).await.unwrap();
    ...
    assert!(after - before >= 6, "expected >= 6 audit rows, got {} -> {}", before, after);
}
...
async fn list_members_hides_soft_deleted_users(pool: PgPool) {
    ...
    let s = svc(pool);
    let members = s.list_members(tenant(), team_id).await.unwrap();
    assert!(!members.iter().any(|m| m.user_id == admin_id),
...
async fn platform_admin_in_default_tenant_does_not_see_other_tenants_teams(pool: PgPool) {
    ...
    let _team_b = s.create_team(TenantId::from_trusted(tenant_b),
        &create_req("Distant"), &ctx()).await.unwrap();
    // Ask as DEFAULT_TENANT_ID (where the platform admin sits).
    let default_view = s.list_teams(tenant(), TeamListFilters::default()).await.unwrap();
...
async fn list_teams_manager_id_filter(pool: PgPool) {
    ...
    s.create_team(tenant(), &create_req("NoMgr"), &ctx()).await.unwrap();

    let filtered = s.list_teams(tenant(), TeamListFilters {
        manager_id: Some(admin_id), ..Default::default()
    }).await.unwrap();
...
async fn personal_tenant_can_technically_create_team_but_ui_hides_it(pool: PgPool) {
    ...
    .bind(personal_id).bind("Personal Test").bind("personal-teams-32")
    .bind(Uuid::new_v4())
    .execute(&pool).await.unwrap();

    let s = svc(pool);
    let team = s.create_team(TenantId::from_trusted(personal_id),
        &create_req("PersonalOK"), &AuditCtx::system(personal_id)).await
...
async fn update_team_clearing_manager_id_with_null(pool: PgPool) {
    ...
    let (other_id, _e2, _p2) = common::seed_user(
        &pool, common::DEFAULT_TENANT_ID, "other@example.com", "manager").await;
    let s = svc(pool);
    ...
    s.update_team(tenant(), team.id, &UpdateTeamRequest {
        ...
        name: None, description: None, color: None, is_active: None,
    }, &ctx()).await.unwrap();
...
async fn add_member_to_archived_team_succeeds(pool: PgPool) {
    let (admin_id, _e, _p) = common::seed_admin(&pool).await;
    let s = svc(pool);
    let team = s.create_team(tenant(), &create_req("Bravo34"), &ctx()).await.unwrap();
    s.soft_delete_team(tenant(), team.id, &ctx()).await.unwrap();
    // No server-side block; documents the accepted race per Q9.
    s.add_member(tenant(), team.id, &AddTeamMemberRequest {
        user_id: admin_id, role: None,
    }, &ctx()).await.expect("no block on archived-team add");
}
...
async fn existing_tickets_with_null_team_id_read_back_ok(pool: PgPool) {
    ...
    let team_col: Option<Uuid> =
        sqlx::query_scalar("SELECT team_id FROM tickets WHERE id = $1")
...
async fn team_delete_of_team_with_tickets_soft_deletes_not_hard(pool: PgPool) {
    let s = svc(pool.clone());
    let team = s.create_team(tenant(), &create_req("Regress36"), &ctx()).await.unwrap();
    ...
    s.soft_delete_team(tenant(), team.id, &ctx()).await.expect("soft delete succeeds");
    let read_team = s.get_team(tenant(), team.id).await.unwrap();
    assert!(!read_team.is_active, "team is soft-deleted (is_active=false)");
    let ticket_team: Option<Uuid> =
        sqlx::query_scalar("SELECT team_id FROM tickets WHERE id = $1")
            .bind(ticket_id).fetch_one(&pool).await.unwrap();
