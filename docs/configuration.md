...
| `AUTH_PROVIDERS` | `.env` | Comma-separated authentication-provider priority list. Unset reproduces the hosting profile's default (`bunyip,local` in `saas`, `local` in `self-hosted`). Landed dormant (PMS-981): both paths are named but `create_api_router` still mounts both middlewares directly, so no request is routed through this list yet. |
