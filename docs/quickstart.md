...
- Rust `1.98.1` (`cargo`, `rustc`), the exact patch `rust-toolchain.toml` pins so rustfmt and clippy match CI; rustup reads that file and fetches it for you. The server itself compiles inside the `server` container, but the host toolchain runs `just check`, `just infisical-bootstrap` and any `cargo` command you run directly.
