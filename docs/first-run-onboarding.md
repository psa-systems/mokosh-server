...
4. Name the deployment as that admin: `PUT /api/v1/settings/app-name` (admin-only, `RequireAdmin`; PMS-789), body `{"app_name": "PSA Systems"}`. This is the product name recipients see in invitation and test mail, in the authenticator app when they enrol in MFA, and on the API's own landing page. It is not what the password-reset and welcome mail say: those carry the tenant's own MSP identity (`{{msp_name}}` and its branding), so a customer receiving one sees the MSP they hired, not the software (PMS-1124). It is one value for the whole deployment, not per tenant, and it takes effect immediately with no restart. Leave it unset and everything reads `Mokosh`; send an empty string to clear it back to that. It is deliberately not an environment variable, so changing it never needs a redeploy.
