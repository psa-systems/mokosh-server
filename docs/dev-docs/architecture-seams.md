...
| Platform, legacy password | `src/modules/auth/service.rs` + `middleware.rs` | `/api/v1/auth/*` | HS256 JWT as an Authorization-header Bearer token, Argon2 password, TOTP (`src/utils/totp.rs`) and recovery codes (`src/utils/recovery.rs`) | `public.user_sessions`, plus the credential columns on `public.users` |
