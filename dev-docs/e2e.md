...
1. **E2E tenant** - dedicated tenant; record its UUID as `E2E_TENANT_ID`.
2. **E2E account** - a user in that tenant with the **`admin`** role (or `super_admin`). Admin is REQUIRED, not optional: the suite enables gated modules and many writes are `RequireAdmin`/`RequireManager`/`RequireFinance` (admin satisfies all three). The role lives in mokosh's local `users` row, NOT in the bunyip token; first SSO login JIT-creates the row as `technician`, so elevate it: `UPDATE users SET role = 'admin' WHERE email = '<E2E_EMAIL>'` on the staging mokosh Postgres. The role is re-read per request, so no re-login is needed. Enable 2FA and record the base32 secret as `E2E_TOTP_SECRET`.
...
The runner uses Node + Chromium off the OpenSUSE base image (`node24` binary, `corepack npm` / `corepack npx`; see Troubleshooting). On failure it uploads `playwright-report/` + `test-results/` as artifacts.
...
**Everything admin-gated returns 403.** The E2E account is not admin. Confirmed on the first PMS-155 run: a non-admin `E2E_EMAIL` passed tickets/contacts/OIDC but 403'd on all 14 module specs. Elevate the row: `UPDATE users SET role = 'admin' WHERE email = '<E2E_EMAIL>'` on staging mokosh Postgres (role is resolved from the local `users` row, not the bunyip token).
